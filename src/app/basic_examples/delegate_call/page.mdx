...
For scenarios requiring untyped calls to another contract, `RawCall` offers a configurable and fluent API. You can set up an unsafe delegate call by chaining optional configuration methods, similar to configuring a `File` in Rust.
...
### Benefits of Using `RawCall`

- **Configurable**: Offers granular control over the call parameters.
- **Fluent API**: Enables chaining methods for clearer and more maintainable code.
- **Safety Awareness**: While still unsafe, the configuration methods make it explicit what the call will do.
