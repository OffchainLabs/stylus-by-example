...
And part of the `Cargo.toml` file, notice that it has the `export-abi` feature flag enabled.
...
You can also obtain a JSON formatted ABI using the parameter `--json`.
...
Additionally, when using argument names like `address`, the SDK will prepend an `_` when generating the Solidity ABI interface. For example:
