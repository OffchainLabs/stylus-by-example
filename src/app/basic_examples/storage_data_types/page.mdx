...
| Type             | Description                                                                  |
|-----------------|------------------------------------------------------------------------------|
| `StorageBool`    | Stores a boolean value.                                                      |
| `StorageAddress` | Stores an Ethereum address (Alloy Address).                                  |
| `StorageUint`    | Stores an unsigned integer (Alloy Uint).                                      |
| `StorageSigned`  | Stores a signed integer (Alloy Signed).                                       |
| `StorageFixedBytes`| Stores a fixed-size byte array (Alloy FixedBytes).                                |
| `StorageVec`     | Stores a dynamically sized vector (array) of `StorageType`.                   |
| `StorageString`  | Stores a UTF-8 encoded string (Solidity `string`). This is a wrapper around `StorageBytes`. |
| `StorageBytes`   | Stores a dynamic byte array (Solidity `bytes`).                               |
| `StorageMap`     | Stores a mapping of `StorageKey` to `StorageType`.                             |
| `StorageArray`   | Stores a fixed-size array of `StorageType`.                                     |

`StorageString` is essentially a wrapper around `StorageBytes`, providing convenient methods for working with UTF-8 encoded strings.  `StorageMap` and `StorageArray` will be discussed in separate sections due to their more complex nature.  This section focuses on the first seven types.
