...
Start by defining a simple counter contract with storage, timestamping, and an event:
...
Implement basic methods that call `vm()` under the hood:
...
impl Counter {
  ...
  pub fn increment(&mut self) {
    ...
    self.set_number(prev + U256::ONE);
  }

  pub fn set_number(&mut self, n: U256) {
    let prev = self.number.get();
    self.number.set(n);
...
This gives you a contract that:

* Increments / sets a number
* Updates a timestamp
* Emits an event with indexed parameters
* Accepts `msg.value()` in a payable path

---

## 3. Basic Unit Test

With `stylus_sdk::testing::*` imported, write tests just like any Rust project:
...
* `TestVM::default()` seeds a clean in-memory VM
...
## 4. Inspecting & Mocking Host I/O

Stylus’s `TestVM` provides methods to override and inspect every host function. Use the table below as a quick reference:

| **Scenario**                    | **TestVM API**                                                         |
| ------------------------------- | ---------------------------------------------------------------------- |
| Override Ether attached         | `vm.set_value(U256)`                                                   |
| Override Caller address         | `vm.set_sender(Address)`                                               |
| Read raw storage slot           | `vm.storage_load_bytes32(slot)`                                        |
| Write raw storage slot & commit | `unsafe { vm.storage_cache_bytes32(slot,val) }; vm.flush_cache(false)` |
| Override block parameters       | `vm.set_block_number(n)`<br>`vm.set_block_timestamp(ts)`               |
| Inspect emitted logs & events   | `vm.get_emitted_logs()`                                                |
| Mock external call response     | `vm.mock_call(addr, data, Ok(res)/Err(revert))`                        |
...
To verify events and their indexed parameters:
...
## 6. Mocking External Calls

Stub out `vm().call(...)` to test cross-contract interactions:
...
## 7. Raw Storage Testing

Directly inspect or override any storage slot:
...
* Ideal for testing storage layout, mappings, or verifying internal state after corner-case paths.

---

## 8. Block-Context Testing

Simulate block-dependent logic by overriding block number/timestamp:
...
* Good for timelocks, expiry logic, or height-based gating.
* `TestVMBuilder` seeds initial values; `vm.set_*` mutates them mid-test.

---

## 9. Customizing the VM

### 9-a. `TestVMBuilder`

Pre-seed context before deploying:
...
## 10. Cheat-Sheet & Best Practices

| **What to test…**      | **TestVM API**                                                                                    |
| ---------------------- | ------------------------------------------------------------------------------------------------- |
| `msg.value()`          | `vm.set_value(U256)`                                                                              |
| `msg.sender()`         | `vm.set_sender(Address)`                                                                          |
| Raw storage            | `vm.storage_load_bytes32(k)`<br>`unsafe { vm.storage_cache_bytes32(k,v) }; vm.flush_cache(false)` |
| Block params           | `vm.set_block_number(n)`<br>`vm.set_block_timestamp(ts)`                                          |
