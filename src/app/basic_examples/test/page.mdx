...
```rust
self.vm().msg_value()              // mocked msg.value() in tests
self.vm().msg_sender()             // mocked msg.sender()
self.vm().block_timestamp()        // mocked block timestamp

// low-level external call (from inside your contract code)
let ctx = Call::new_mutating(self);             // or Call::new() for view
let ret = call(self.vm(), ctx, addr, &data);    // free function, not a VM method
```
